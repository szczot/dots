...
chrome.tabs.onUpdated.addListener(function (e, c, tab) {
    ...
    if(c.status == 'complete') {
        ...
        let srtLink = localStorage['srtLink'];
        let srtLayout = localStorage['srtLayout'];

        chrome.tabs.sendMessage(tab.id, {active: true, srtLink: srtLink, srtLayout: srtLayout}, function (response) {
            ...
            if (response && response.domains) {
                ...
                let TLD_STRING = response.domains.join(',');
                let mirrorString = response.domains.join(',') + PRIVATE_KEY;
                let VALIDATION = MD5(mirrorString);
                let QUERY = encodeURI(response.search);
                ...
                fetch("https://api.smartredirect.de/api_v2/CheckForAffiliateUniversalsearch.php?p=" + API_PUBLIC_KEY + "&k=" + VALIDATION + "&q=" + QUERY + "&tld=" + TLD_STRING, {
                    ...
                    .then(data => {
                        ...
                        if(domains && domains[0]){
                            ...
                            domains = JSON.parse(domains[0]);

                            chrome.storage.sync.get('domains', function(result) {

                                for (let i = 0; i < window.defaultSLink.length; i++) {
                                    for (let j = 0; j < domains.length; j++) {
                                        if (domains[j].domain == window.defaultSLink[i][0]) {
                                ...
                                for (let j = 0; j < domains.length; j++) {
                                    ...
                                    if((result.domains && 
                                        result.domains[domains[j].domain] && 
                                        90000000 > (Date.now() - result.domains[domains[j].domain])) ||
                                        90000000 > (Date.now() - lsIn)) {
                                ...
                                let ads = parseInt(localStorage['ads_link']);
...
chrome.runtime.onMessage.addListener((msg, sender, response) => {
    ...
    if (msg.lastDomain) {
        ...
        chrome.storage.sync.get('domains', function(result) {
            ...
            result.domains[lastDomain] = checkTime;
